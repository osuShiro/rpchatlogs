...
class SessionTestCase(TestCase):
    ...
    def setUp(self):
        ...
        for i in range(1, 11):
            message = models.Message(text='message' + str(i), session=session1, timestamp=datetime.datetime.utcnow())
            message.save()
    ...
    def test_session_actions_not_superuser(self):
        ...
        self.assertEqual(CLIENT.patch('/game-admin/public game/session/session1/').status_code, 401)
        self.assertEqual(CLIENT.post('/game-admin/public game/session/session1/').status_code, 401)
        self.assertEqual(CLIENT.delete('/game-admin/public game/session/session1/').status_code, 401)
        self.assertEqual(CLIENT.get('/game-admin/public game/add/').status_code, 401)

    def test_session_patch_delete_no_messages(self):
        self.assertEqual(CLIENT_ADMIN.post('/game-admin/public game/session/session1/',
    ...
    def test_session_patch_delete_selected(self):
        self.assertEqual(CLIENT_ADMIN.post('/game-admin/public game/session/session1/',
                                            ...
                                            HTTP_AUTHORIZATION='JWT {}'.format(login.login('admin'))).status_code,
                         200)
        self.assertEqual(models.Message.objects.count(), 8)
    ...
    def test_session_add_wrong_methods(self):
        self.assertEqual(CLIENT_ADMIN.patch('/game-admin/public game/session/add/').status_code, 405)
        self.assertEqual(CLIENT_ADMIN.delete('/game-admin/public game/session/add/').status_code, 405)

    def test_session_add_no_chatlog(self):
        self.assertEqual(CLIENT_ADMIN.post('/game-admin/public game/add/',
                                           {
                                               'title': 'title',
                                           },
                                           HTTP_AUTHORIZATION='JWT {}'.format(login.login('admin'))).status_code,
    ...
    def test_session_append_logged_out(self):
        self.assertEqual(CLIENT.get('/game-admin/public game/session1/append/').status_code, 302)

    def test_session_append_not_superuser(self):
        CLIENT.login(username='player', password='bla')
        self.assertEqual(CLIENT.get('/game-admin/public game/session1/append/').status_code, 401)

    def test_session_append_wrong_methods(self):
        self.assertEqual(CLIENT_ADMIN.patch('/game-admin/public game/session/session1/append/').status_code, 405)
        self.assertEqual(CLIENT_ADMIN.post('/game-admin/public game/session/session1/append/').status_code, 405)
        self.assertEqual(CLIENT_ADMIN.delete('/game-admin/public game/session/session1/append/').status_code, 405)
